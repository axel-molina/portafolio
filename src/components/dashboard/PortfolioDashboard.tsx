      ...
      >
        <motion.div variants={item}>
          <SummaryCard
            ...
            value={hasPrices ? formatCurrency(totalInvested, 'USD', exchangeRate) : totalInvested > 0 ? formatCurrency(totalInvested, 'USD', exchangeRate) : '—'}
        ...
        <motion.div variants={item}>
          <SummaryCard
            ...
            value={loading ? '—' : hasPrices ? formatCurrency(totalCurrentValue, 'USD', exchangeRate) : '—'}
        ...
        <motion.div variants={item}>
          <SummaryCard
            ...
            value={showProfitLoss ? formatCurrency(totalProfitLoss, 'USD', exchangeRate) : '—'}
            ...
            {allocationData.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  ...
                  <Tooltip 
                    formatter={(value: unknown) => formatCurrency(value as number, 'USD', exchangeRate)}
                    contentStyle={{
                ...
                <BarChart data={assetsByValue} barSize={40}>
                  ...
                  <Tooltip
                    formatter={(value: unknown, name: unknown) => [
                      name === 'value' ? formatCurrency(value as number, 'USD', exchangeRate) : formatPercent(value as number),
                ...
                {assets.map((asset, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '16px 24px', textAlign: 'right', fontSize: '14px', color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)', fontVariantNumeric: 'tabular-nums' }}>
                        {formatCurrency(asset.averagePrice, 'USD', exchangeRate)}
                      </td>
                      <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        <p style={{ fontSize: '14px', color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)', fontVariantNumeric: 'tabular-nums' }}>
                          {hasPriceData ? formatCurrency(currentPrice, 'USD', exchangeRate) : formatCurrency(asset.averagePrice, 'USD', exchangeRate)}
                        </p>
                        {priceData && (
                      ...
                      <td style={{ padding: '16px 24px', textAlign: 'right', fontSize: '14px', fontWeight: 600, color: 'var(--color-text-primary)', fontFamily: 'var(--font-sans)', fontVariantNumeric: 'tabular-nums' }}>
                        {hasPriceData ? formatCurrency(currentValue, 'USD', exchangeRate) : '—'}
                      </td>
                      <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        <p style={{ fontSize: '14px', fontWeight: 600, color: hasPriceData && profitLoss >= 0 ? 'var(--color-success)' : hasPriceData ? 'var(--color-danger)' : 'var(--color-text-muted)', fontFamily: 'var(--font-sans)', fontVariantNumeric: 'tabular-nums' }}>
                          {hasPriceData ? formatCurrency(profitLoss, 'USD', exchangeRate) : '—'}
                        </p>
                        {hasPriceData && (
